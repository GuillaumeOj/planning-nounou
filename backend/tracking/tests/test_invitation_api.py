...
from tracking.models import ContractInvitation, ContractShare

pytestmark = pytest.mark.django_db


def invitations_url(family, contract):
    return reverse("tracking:contract-invitations", args=[family.id, contract.id])
...
def test_preview_is_public_and_hides_token(client, owner, contract):
    ...
    url = reverse("tracking:contract-invitation-preview", args=[invitation.token])
...
def test_accept_attaches_chosen_family(client, owner, outsider, other_family, contract):
    ...
    url = reverse("tracking:contract-invitation-accept", args=[invitation.token])
...
def test_decline(client, owner, outsider, contract):
    invitation = make_invitation(contract, owner, email=outsider.email)
    client.force_authenticate(user=outsider)
    url = reverse("tracking:contract-invitation-decline", args=[invitation.token])
...
def test_revoke_keeps_audit_trail(client, owner, family, contract):
    invitation = make_invitation(contract, owner)
    client.force_authenticate(user=owner)
    url = reverse("tracking:contract-invitation", args=[family.id, contract.id, invitation.id])
...
def test_my_invitations_inbox(client, owner, outsider, contract):
    ...
    resp = client.get(reverse("tracking:my-contract-invitations"))
