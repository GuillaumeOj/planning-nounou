...
def test_list_returns_only_family_children(client, user, family, other_family):
    ...
    resp = client.get(reverse("children:family-children", args=[family.pk]))
...
def test_create_child_assigns_family_from_url(client, user, family):
    ...
    resp = client.post(
        reverse("children:family-children", args=[family.pk]),
        {"first_name": "Leo"},
        format="json",
    )
...
def test_update_child_renames(client, user, family):
    ...
    resp = client.patch(
        reverse("children:family-child", args=[family.pk, child.pk]),
        {"first_name": "Leon"},
        format="json",
    )
...
def test_delete_child_removes_it(client, user, family):
    ...
    resp = client.delete(reverse("children:family-child", args=[family.pk, child.pk]))
...
def test_cannot_access_another_familys_children(client, user, other_family):
    ...
    resp = client.get(reverse("children:family-children", args=[other_family.pk]))
...
def test_unclaimed_family_creator_can_manage_children(client, user):
    ...
    resp = client.post(
        reverse("children:family-children", args=[fam.pk]),
        {"first_name": "Ada"},
        format="json",
    )
...
def test_children_require_authentication(client, family):
    assert client.get(reverse("children:family-children", args=[family.pk])).status_code == 401
