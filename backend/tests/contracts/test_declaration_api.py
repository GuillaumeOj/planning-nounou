...
from accounts.models import Family, FamilyMembership
...
def hour_url(family, contract, pk):
    return reverse("contracts:contract-exceptional-hour", args=[family.id, contract.id, pk])


def declarations_url(family, contract):
    return reverse("contracts:contract-declarations", args=[family.id, contract.id])


def file_url(family, contract, pk):
    return reverse("contracts:contract-declaration-file", args=[family.id, contract.id, pk])
...
def test_windows_are_replaced_wholesale_on_update(client, owner, family, wired):
    ...
    url = reverse("contracts:contract-child", args=[family.id, wired.id, link.id])
    resp = client.patch(
        url,
...
def test_a_co_employers_presence_is_no_longer_editable_once_claimed(client, owner, outsider, wired):
    ...
    url = reverse("contracts:contract-child", args=[b.id, wired.id, link.id])
...
def test_kilometres_can_be_typed_and_repriced(client, owner, family, wired):
    ...
    url = reverse("contracts:contract-declaration", args=[family.id, wired.id, row["id"]])
...
def test_a_family_does_not_read_the_other_familys_declaration(
    ...
    detail = reverse("contracts:contract-declaration", args=[family.id, shared.id, theirs.id])
