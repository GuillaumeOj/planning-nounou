...
def child(family_id, *windows, child_id=None):
    return declarations.ChildPresence(
        child_id=child_id or uuid4(),
        family_id=family_id,
        windows=tuple(declarations.Window(day, start, end) for day, start, end in windows),
    )
...
def block(weekday=MONDAY, start=time(8, 0), end=time(18, 0)):
    return declarations.Block(weekday=weekday, start=start, end=end)


def schedule(*blocks, effective_from=date(2026, 1, 1)):
    return declarations.Schedule(effective_from=effective_from, blocks=tuple(blocks))


def terms(rate="12.00", effective_from=date(2026, 1, 1), **kwargs):
    return declarations.Terms(
...
def test_a_child_with_no_window_is_always_present():
    assert declarations.presence_on(child(FAMILY_A), date(2026, 7, 13)) is None
...
def test_windows_are_read_across_weekdays_not_within_one():
    ...
    assert declarations.presence_on(kid, date(2026, 7, 15)) == ()  # a Wednesday: absent
    assert declarations.presence_on(kid, date(2026, 7, 13)) != ()  # a Monday: there


def test_an_override_adds_presence_for_one_date_only():
    kid = child(FAMILY_A, (MONDAY, time(16, 30), time(18, 0)))
    override = declarations.PresenceOverride(
        kid.child_id, date(2026, 7, 13), time(8, 0), time(12, 0)
    )
    assert declarations.presence_on(kid, date(2026, 7, 13), [override]) == (
        declarations.Interval(time(8, 0), time(12, 0)),
        declarations.Interval(time(16, 30), time(18, 0)),
    )
    # A different Monday keeps the regular window.
    assert declarations.presence_on(kid, date(2026, 7, 20)) == (
        declarations.Interval(time(16, 30), time(18, 0)),
    )


def test_covers_needs_the_whole_segment_not_just_its_start():
    intervals = (declarations.Interval(time(8, 0), time(12, 0)),)
    assert declarations.covers(intervals, time(9, 0), time(10, 0))
    assert not declarations.covers(intervals, time(11, 0), time(13, 0))
...
def test_a_window_reaching_past_the_block_does_not_cut_a_phantom_segment():
    ...
    segments = declarations.segment_block(
        block(), {kid.child_id: declarations.presence_on(kid, date(2026, 7, 13))}
    )
...
def test_no_child_present_falls_back_to_an_equal_split():
    ...
    weights = declarations.segment_weights(frozenset(), {}, "by_children", [FAMILY_A, FAMILY_B])
    assert weights == {FAMILY_A: Fraction(1, 2), FAMILY_B: Fraction(1, 2)}


def test_a_solo_contract_never_divides_by_zero():
    assert declarations.segment_weights(frozenset(), {}, "equal", [FAMILY_A]) == {
        FAMILY_A: Fraction(1)
    }
...
def test_equal_counts_families_present_not_families_on_the_contract():
    ...
    weights = declarations.segment_weights(
        frozenset({a.child_id}), children, "equal", [FAMILY_A, FAMILY_B]
    )
...
def test_by_children_weighs_each_family_by_its_children_present():
    ...
    weights = declarations.segment_weights(present, children, "by_children", [FAMILY_A, FAMILY_B])
...
def test_the_same_families_split_equally_when_they_agreed_to():
    ...
    children = by_id(a1, a2, b1)
    present = frozenset({a1.child_id, a2.child_id, b1.child_id})
    weights = declarations.segment_weights(present, children, "equal", [FAMILY_A, FAMILY_B])
    assert weights == {FAMILY_A: Fraction(1, 2), FAMILY_B: Fraction(1, 2)}
...
def test_weights_always_sum_to_one():
    ...
    for method in ("equal", "by_children"):
        for present in (
            ...
            weights = declarations.segment_weights(present, children, method, [FAMILY_A, FAMILY_B])
...
def test_the_after_school_child_shifts_the_split_mid_day():
    ...
    b1 = child(FAMILY_B)
    children = by_id(a1, a2, b1)
    week = declarations.band_week(schedule(block()), children, "by_children", [FAMILY_A, FAMILY_B])

    assert declarations.to_hours(week.total[FAMILY_A].total) == Decimal("5.25")
    assert declarations.to_hours(week.total[FAMILY_B].total) == Decimal("4.75")
...
def test_the_week_is_banded_before_it_is_split():
    ...
    week = declarations.band_week(schedule(*blocks), by_id(a, b), "equal", [FAMILY_A, FAMILY_B])

    combined = week.total[FAMILY_A] + week.total[FAMILY_B]
    assert combined.normal == declarations.WEEKLY_NORMAL_MINUTES
...
def test_the_50_percent_band_opens_after_eight_overtime_hours():
    assert declarations.allocate_bands(0, 50 * 60) == [
        (declarations.BAND_NORMAL, 40 * 60),
        (declarations.BAND_25, 8 * 60),
        (declarations.BAND_50, 2 * 60),
    ]


def test_a_week_under_the_threshold_is_all_normal():
    assert declarations.allocate_bands(0, 35 * 60) == [(declarations.BAND_NORMAL, 35 * 60)]
...
def test_mensualisation_spreads_the_week_over_twelve_months():
    ...
    bands = declarations.Bands(normal=Fraction(40 * 60))
    assert declarations.to_hours(declarations.mensualise(bands).normal) == Decimal("173.33")
...
def test_a_month_with_one_snapshot_is_one_period_weighing_the_whole_month():
    ...
    periods = declarations.sub_periods(data)
...
def test_a_contract_starting_mid_month_is_weighed_against_the_calendar_month():
    ...
    assert sum(p.weight for p in declarations.sub_periods(data)) == Fraction(16, 31)
...
def test_a_month_before_the_contract_starts_yields_nothing():
    ...
    assert declarations.sub_periods(data) == []
...
def test_a_solo_entry_is_wholly_its_filers():
    ...
    entry = declarations.ExceptionalEntry(
        FAMILY_A, "effective", date(2026, 7, 14), time(20, 0), date(2026, 7, 14), time(23, 0)
    )
    minutes = declarations.attribute_exceptional([entry], EQUAL_SHARES, [FAMILY_A, FAMILY_B])
...
def test_a_family_filing_overlapping_solo_entries_is_not_paid_for_both():
    ...
    minutes = declarations.attribute_exceptional(entries, EQUAL_SHARES, [FAMILY_A, FAMILY_B])
...
def test_a_shared_entry_splits_by_children_when_the_contract_says_so():
    ...
    b1 = child(FAMILY_B)
    children = by_id(a1, a2, b1)
    shares = declarations.contract_shares(children, "by_children", [FAMILY_A, FAMILY_B])
    entry = declarations.ExceptionalEntry(
        FAMILY_A,
        "effective",
        date(2026, 7, 14),
    ...
    minutes = declarations.attribute_exceptional([entry], shares, [FAMILY_A, FAMILY_B])
...
def test_solo_entries_never_read_the_childrens_windows():
    ...
    b = child(FAMILY_B)
    shares = declarations.contract_shares(by_id(a, b), "equal", [FAMILY_A, FAMILY_B])
    entry = declarations.ExceptionalEntry(
        FAMILY_A, "effective", date(2026, 7, 14), time(19, 0), date(2026, 7, 14), time(21, 0)
    )
    minutes = declarations.attribute_exceptional([entry], shares, [FAMILY_A, FAMILY_B])
...
def test_a_night_crossing_midnight_is_one_span():
    ...
    minutes = declarations.attribute_exceptional(entries, {FAMILY_A: Fraction(1)}, [FAMILY_A])
...
def test_apportion_parts_sum_to_the_whole():
    ...
    parts = declarations.apportion(Decimal("10"), [Fraction(1, 3)] * 3)
...
def test_apportion_handles_a_single_share():
    assert declarations.apportion(Decimal("40"), [Fraction(1)]) == [Decimal("40")]


def test_apportion_with_no_weight_pays_nobody():
    assert declarations.apportion(Decimal("40"), [Fraction(0), Fraction(0)]) == [
        Decimal("0"),
        Decimal("0"),
    ]
...
def test_apportion_never_invents_or_loses_money(total):
    parts = declarations.apportion(Decimal(total), [Fraction(1, 7), Fraction(3, 7), Fraction(3, 7)])
...
def test_a_solo_forty_hour_week_mensualises_to_the_urssaf_figure():
    ...
    result = declarations.compute_month(month(schedules=(schedule(*blocks),)))[FAMILY_A]
...
def test_a_shared_month_declares_at_least_what_the_nanny_worked():
    ...
    b1 = child(FAMILY_B)
    results = declarations.compute_month(
    ...
    worked = declarations.to_hours(
        declarations.mensualise(declarations.Bands(Fraction(50 * 60))).normal
    )
...
def test_unpaid_leave_prorates_the_month_rather_than_subtracting_the_day():
    ...
    blocks = [block(day, time(9, 0), time(17, 0)) for day in range(5)]
    leave = declarations.LeaveSpan("unpaid", date(2026, 7, 13), date(2026, 7, 13), "full_day")
    without = declarations.compute_month(month(schedules=(schedule(*blocks),)))[FAMILY_A]
    with_leave = declarations.compute_month(month(schedules=(schedule(*blocks),), leaves=(leave,)))[
        FAMILY_A
    ]
...
def test_paid_leave_deducts_nothing_because_the_base_already_holds_it():
    ...
    leave = declarations.LeaveSpan("paid", date(2026, 7, 13), date(2026, 7, 17), "full_day")
    without = declarations.compute_month(month())[FAMILY_A]
    with_leave = declarations.compute_month(month(leaves=(leave,)))[FAMILY_A]
...
def test_leave_on_a_day_the_nanny_never_works_deducts_nothing():
    ...
    leave = declarations.LeaveSpan(
        "unpaid", date(2026, 7, 14), date(2026, 7, 14), "full_day"
    )  # a Tuesday
    without = declarations.compute_month(month(schedules=(schedule(*blocks),)))[FAMILY_A]
    with_leave = declarations.compute_month(month(schedules=(schedule(*blocks),), leaves=(leave,)))[
        FAMILY_A
...
def test_a_month_of_unpaid_leave_is_worth_nothing_whatever_the_month(first, last):
    ...
    leave = declarations.LeaveSpan("unpaid", first, last, "full_day")
    data = declarations.ContractMonth(
        month=first,
        starting_date=date(2026, 1, 5),
        ending_date=None,
    ...
    result = declarations.compute_month(data)[FAMILY_A]
...
def test_sickness_deducts_the_hours_like_an_unpaid_absence():
    ...
    blocks = [block(day, time(9, 0), time(17, 0)) for day in range(5)]
    sick = declarations.LeaveSpan("sickness", date(2026, 7, 13), date(2026, 7, 13), "full_day")
    unpaid = declarations.LeaveSpan("unpaid", date(2026, 7, 13), date(2026, 7, 13), "full_day")
    with_sick = declarations.compute_month(month(schedules=(schedule(*blocks),), leaves=(sick,)))[
        FAMILY_A
    ]
    with_unpaid = declarations.compute_month(
        month(schedules=(schedule(*blocks),), leaves=(unpaid,))
    )[FAMILY_A]
    without = declarations.compute_month(month(schedules=(schedule(*blocks),)))[FAMILY_A]
...
def test_maternity_deducts_the_hours_like_a_sick_absence():
    ...
    blocks = [block(day, time(9, 0), time(17, 0)) for day in range(5)]
    maternity = declarations.LeaveSpan(
        "maternity", date(2026, 7, 13), date(2026, 7, 13), "full_day"
    )
    sick = declarations.LeaveSpan("sickness", date(2026, 7, 13), date(2026, 7, 13), "full_day")
    with_maternity = declarations.compute_month(
        month(schedules=(schedule(*blocks),), leaves=(maternity,))
    )[FAMILY_A]
    with_sick = declarations.compute_month(month(schedules=(schedule(*blocks),), leaves=(sick,)))[
        FAMILY_A
    ]
    ...
    assert (
        ...
        in declarations.compute_month(month(leaves=(maternity,)))[FAMILY_A].warnings
    )
...
def test_a_sick_day_shares_its_reduction_across_the_families():
    ...
    a, b = child(FAMILY_A), child(FAMILY_B)
    sick = declarations.LeaveSpan(
        "sickness", date(2026, 7, 13), date(2026, 7, 13), "full_day"
    )  # a Monday
    plain = declarations.compute_month(month(children=(a, b), families=(FAMILY_A, FAMILY_B)))
    sickened = declarations.compute_month(
...
def test_a_deducting_absence_is_flagged_but_paid_leave_is_not():
    ...
    sick = declarations.LeaveSpan("sickness", date(2026, 7, 13), date(2026, 7, 13), "full_day")
    unpaid = declarations.LeaveSpan("unpaid", date(2026, 7, 13), date(2026, 7, 13), "full_day")
    paid = declarations.LeaveSpan("paid", date(2026, 7, 13), date(2026, 7, 13), "full_day")
    assert (
        "hours_reduced_for_absence"
        in declarations.compute_month(month(leaves=(sick,)))[FAMILY_A].warnings
    )
    assert (
        "hours_reduced_for_absence"
        in declarations.compute_month(month(leaves=(unpaid,)))[FAMILY_A].warnings
    )
    assert (
        "hours_reduced_for_absence"
        not in declarations.compute_month(month(leaves=(paid,)))[FAMILY_A].warnings
    )
    assert "hours_reduced_for_absence" not in declarations.compute_month(month())[FAMILY_A].warnings
...
def test_exceptional_hours_add_to_the_month():
    ...
    without = declarations.compute_month(month())[FAMILY_A]
    with_extra = declarations.compute_month(month(exceptional=(entry,)))[FAMILY_A]
...
def test_a_night_is_an_indemnity_not_hours():
    ...
    plain = declarations.compute_month(month())[FAMILY_A]
    result = declarations.compute_month(
...
def test_an_unset_night_rate_still_pays_the_floor():
    ...
    result = declarations.compute_month(month(terms_=(terms("12.00"),), exceptional=(entry,)))[
        FAMILY_A
    ]
    assert result.night_indemnity == Decimal("12.00")
...
def test_a_longer_night_costs_more_because_the_indemnity_is_hourly():
    ...
    short = declarations.ExceptionalEntry(
        FAMILY_A, "night_presence", date(2026, 7, 14), time(22, 0), date(2026, 7, 15), time(0, 0)
    )
    long = declarations.ExceptionalEntry(
        FAMILY_A, "night_presence", date(2026, 7, 14), time(22, 0), date(2026, 7, 15), time(4, 0)
    )
    rate = (terms("12.00", night_presence_rate="3.00"),)
    two = declarations.compute_month(month(terms_=rate, exceptional=(short,)))[FAMILY_A]
    six = declarations.compute_month(month(terms_=rate, exceptional=(long,)))[FAMILY_A]
...
def test_a_night_woken_twice_is_owed_a_third_not_a_quarter():
    ...
    quiet = declarations.ExceptionalEntry(
        FAMILY_A, "night_presence", date(2026, 7, 14), time(21, 0), date(2026, 7, 15), time(1, 0)
    )
    disturbed = declarations.ExceptionalEntry(
        FAMILY_A,
        "night_presence",
        date(2026, 7, 14),
        time(21, 0),
    ...
    rate = (terms("12.00"),)
    assert declarations.compute_month(month(terms_=rate, exceptional=(quiet,)))[
        FAMILY_A
    ].night_indemnity == Decimal("12.00")  # 4h x 12/4
    assert declarations.compute_month(month(terms_=rate, exceptional=(disturbed,)))[
        FAMILY_A
...
def test_a_mid_month_raise_keeps_the_hours_and_prices_the_declared_ones():
    ...
    flat = declarations.compute_month(month())[FAMILY_A]
    raised = declarations.compute_month(
        month(terms_=(terms("12.00"), terms("13.00", effective_from=date(2026, 7, 16))))
    )[FAMILY_A]
    assert raised.normal_hours == flat.normal_hours
    assert "rates_changed_mid_month" in raised.warnings
    at_new = declarations.compute_month(month(terms_=(terms("13.00"),)))[FAMILY_A]
...
def test_a_month_with_no_schedule_yields_zeroes_rather_than_an_error():
    result = declarations.compute_month(month(schedules=()))[FAMILY_A]
    assert result.normal_hours == Decimal("0")
    assert result.total_amount == Decimal("0")
...
def test_the_families_together_never_declare_less_than_the_nanny_worked(end_hour, split, kids):
    ...
    worked = declarations.to_hours(
        declarations.mensualise(declarations.Bands(Fraction(weekly))).normal
    )
    assert declared >= worked
...
def test_the_declaration_does_not_depend_on_the_order_rows_arrive_in():
    e1 = declarations.ExceptionalEntry(
        FAMILY_A, "effective", date(2026, 7, 14), time(19, 0), date(2026, 7, 14), time(21, 0)
    )
    e2 = declarations.ExceptionalEntry(
        FAMILY_A, "effective", date(2026, 7, 16), time(19, 0), date(2026, 7, 16), time(21, 0)
    )
    forwards = declarations.compute_month(month(exceptional=(e1, e2)))[FAMILY_A]
    backwards = declarations.compute_month(month(exceptional=(e2, e1)))[FAMILY_A]
...
def test_an_entry_outside_the_month_is_not_paid():
    away = declarations.ExceptionalEntry(
        FAMILY_A, "effective", date(2026, 12, 24), time(19, 0), date(2026, 12, 24), time(23, 0)
    )
    plain = declarations.compute_month(month())[FAMILY_A]
    result = declarations.compute_month(month(exceptional=(away,)))[FAMILY_A]
...
def test_an_entry_after_the_contract_ends_is_not_paid():
    ...
    plain = declarations.compute_month(ended_contract(()))[FAMILY_A]
    result = declarations.compute_month(ended_contract((entry,)))[FAMILY_A]
    assert result.total_amount == plain.total_amount
...
def test_an_exceptional_presence_moves_the_split_without_moving_the_hours():
    ...
    override = declarations.PresenceOverride(tom, date(2026, 7, 13), time(8, 0), time(16, 30))
    _, with_data = shared_month(overrides=(override,))

    plain = declarations.compute_month(plain_data)
    shifted = declarations.compute_month(with_data)
...
def test_an_override_applies_to_its_date_and_not_every_matching_weekday():
    ...
    one = declarations.compute_month(one_monday)[FAMILY_A].normal_hours
    two = declarations.compute_month(two_mondays)[FAMILY_A].normal_hours
...
def test_an_override_outside_the_month_changes_nothing():
    ...
    assert (
        declarations.compute_month(away_data)[FAMILY_A].normal_hours
        == declarations.compute_month(plain_data)[FAMILY_A].normal_hours
    )
...
def bastille_day(**kw):
    ...
    return declarations.Holiday(day=date(2026, 7, 14), **kw)
...
def test_a_chomé_holiday_owes_nothing_because_the_base_already_pays_it():
    ...
    plain = declarations.compute_month(month())[FAMILY_A]
    holiday = declarations.compute_month(month(holidays=(bastille_day(),)))[FAMILY_A]
...
def test_a_worked_holiday_owes_ten_percent_on_the_hours_done():
    ...
    result = declarations.compute_month(month(holidays=(bastille_day(is_workable=True),)))[FAMILY_A]
...
def test_a_worked_first_of_may_owes_a_hundred_percent():
    ...
    may = declarations.ContractMonth(
        month=date(2026, 5, 1),
        starting_date=date(2026, 1, 5),
        ending_date=None,
        split_method="equal",
        ...
        schedules=(schedule(*[block(i) for i in range(5)]),),
        terms=(terms(),),
        holidays=(declarations.Holiday(day=date(2026, 5, 1), is_workable=True),),
    )
    assert declarations.compute_month(may)[FAMILY_A].holiday_majoration == Decimal("120.00")
...
def test_the_journee_de_solidarite_is_worked_and_owes_nothing():
    ...
    result = declarations.compute_month(
        month(holidays=(bastille_day(is_workable=True, is_solidarity=True),))
    )[FAMILY_A]
...
def test_a_holiday_on_a_day_she_never_works_owes_nothing():
    ...
    result = declarations.compute_month(
        month(holidays=(declarations.Holiday(day=date(2026, 8, 15), is_workable=True),))
    )[FAMILY_A]
    assert result.holiday_majoration == Decimal("0")
...
def test_the_majoration_reaches_the_total():
    plain = declarations.compute_month(month())[FAMILY_A]
    worked = declarations.compute_month(month(holidays=(bastille_day(is_workable=True),)))[FAMILY_A]
...
def evening(family, start, end, kind="effective", day=14, shared=False):
    return declarations.ExceptionalEntry(
        family,
        kind,
        date(2026, 7, day),
        time(start, 0),
...
def test_both_families_filing_a_matching_shared_evening_pay_for_it_once():
    ...
    plain = declarations.compute_month(two_family_month())
    shared = declarations.compute_month(two_family_month(exceptional=both))
...
def test_two_families_filing_solo_for_the_same_evening_each_pay_full_and_are_warned():
    ...
    plain = declarations.compute_month(two_family_month())
    result = declarations.compute_month(two_family_month(exceptional=both))
...
def test_shared_care_plus_a_solo_extension_attributes_each_part_on_its_own():
    ...
    plain = declarations.compute_month(two_family_month())
    shared = declarations.compute_month(two_family_month(exceptional=entries))
...
def test_one_family_filing_alone_still_carries_all_of_it():
    plain = declarations.compute_month(two_family_month())
    solo = declarations.compute_month(two_family_month(exceptional=(evening(FAMILY_A, 18, 20),)))
...
def test_a_family_filing_the_same_evening_twice_is_not_paid_for_both():
    ...
    plain = declarations.compute_month(two_family_month())
    result = declarations.compute_month(two_family_month(exceptional=sloppy))
...
def test_nights_and_evenings_do_not_reconcile_with_each_other():
    ...
    plain = declarations.compute_month(two_family_month(terms_=(terms("12.00"),)))
    result = declarations.compute_month(
        two_family_month(terms_=(terms("12.00"),), exceptional=entries)
    )
...
def test_a_correction_on_a_late_week_day_reaches_the_overtime_band():
    ...
    children = by_id(a1, a2, b1)
    blocks = [block(day, time(8, 0), time(17, 0)) for day in range(5)]  # 45h
    friday = declarations.PresenceOverride(a2.child_id, date(2026, 7, 17), time(8, 0), time(16, 0))
    data = month(
        children=(a1, a2, b1),
        schedules=(schedule(*blocks),),
        split="by_children",
    ...
    corrections = declarations.presence_corrections(data, children)
...
def test_every_emitted_warning_has_a_source_and_no_source_is_dead():
    ...
    from contracts.sources import WARNING_SOURCES

    module_source = Path(declarations.__file__).read_text()
